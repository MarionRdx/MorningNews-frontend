...
import { ShowHiddenArticles } from '../reducers/hiddenArticles';
...
function Header() {
	...
	const handleRegister = () => {
		fetch('https://morning-news-backend-theta.vercel.app/users/signup', {
			method: 'POST',
	...
	const handleConnection = () => {
		...
		fetch('https://morning-news-backend-theta.vercel.app/users/signin', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	let handleEyeClickToShow = () => {
		dispatch(ShowHiddenArticles())
	}
	...
	if (user.token) {
		userSection = (
			<div className={styles.logoutSection}>
				...
				<FontAwesomeIcon onClick={() => handleEyeClickToShow()} className={styles.userSection} icon={faEye}/>
			</div>
		);
	} else {
		if (isModalVisible) {
